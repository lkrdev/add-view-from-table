...
const LkrLoading: React.FC<LkrLoadingProps> = ({
    ...
    const [animation_phase, setAnimationPhase] = useState<
    ...
    useEffect(() => {
        ...
        const animate = (currentTime: number) => {
            ...
            const total_duration = duration * 3; // Total time for all phases
    ...
    const renderLetter = (letter: 'L' | 'K' | 'R', index: number) => {
        ...
        const letter_progress = Math.max(
            0,
        ...
        let filter = `drop-shadow(0 0 ${2 + letter_progress * 3}px ${color})`;
        let fill = color;
        let stroke = color;

        // Neon animation style
        if (animation_phase === 'drawing') {
            stroke_dasharray = '1000';
            stroke_dashoffset = `${1000 - letter_progress * 1000}`;
            fill_opacity = 0;
            stroke_opacity = letter_progress;
        } else if (animation_phase === 'filling') {
            stroke_dasharray = '1000';
            stroke_dashoffset = '0';
            fill_opacity = letter_progress;
            stroke_opacity = 1;
        } else {
            stroke_dasharray = '1000';
            stroke_dashoffset = `${letter_progress * 1000}`;
            fill_opacity = 1 - letter_progress;
            stroke_opacity = 1 - letter_progress;
        }
